...
impl<'a> FrontendBytes<'a> {
        ...
        B: BackendType,
    {
        ...
        if m.match_len == 0 {
            ...
            // Unlikely.
            m.idx = item.idx;
            let match_len_dec = self.match_dec(m.idx, m.match_idx);
            m.idx -= match_len_dec;
            ...
            debug_assert!(usize::from(m.idx + m.match_len) <= self.bytes.len());
            m
        }
    }
    ...
    unsafe fn match_unit<M: MatchUnit>(&self, item: UIdx, match_item: UIdx) -> u32 {
        debug_assert!((item.u ^ self.get_u32(item.idx)) & M::MATCH_MASK == 0);
        debug_assert!((match_item.u ^ self.get_u32(match_item.idx)) & M::MATCH_MASK == 0);
    ...
    #[inline(always)]
    unsafe fn match_dec(&self, idx: Idx, match_idx: Idx) -> u32 {
        debug_assert!(match_idx < idx);
        debug_assert!(usize::from(idx) < self.bytes.len());
