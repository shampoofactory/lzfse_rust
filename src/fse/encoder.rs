...
impl Encoder {
    ...
    #[inline(always)]
    pub unsafe fn l<T>(&self, writer: &mut BitWriter<T>, state: &mut L, v: LiteralLen<Fse>)
    where
        T: BitDst,
    {
    ...
    #[inline(always)]
    pub unsafe fn m<T>(&self, writer: &mut BitWriter<T>, state: &mut M, v: MatchLen<Fse>)
    where
        T: BitDst,
    {
    ...
    #[inline(always)]
    pub unsafe fn d<T>(&self, writer: &mut BitWriter<T>, state: &mut D, v: MatchDistancePack<Fse>)
    where
        T: BitDst,
    {
    ...
    #[inline(always)]
    pub unsafe fn u<T>(&self, writer: &mut BitWriter<T>, state: &mut U, u: u8)
    where
        T: BitDst,
    {
...
create_state_struct!(L, L_STATES, Error::BadLmdState);
create_state_struct!(M, M_STATES, Error::BadLmdState);
create_state_struct!(D, D_STATES, Error::BadLmdState);
create_state_struct!(U, U_STATES, Error::BadLiteralState);
...
mod tests {
    ...
    fn test_null() -> io::Result<()> {
        ...
        for _ in 0..32 {
            unsafe { table[0].encode(&mut wtr, &mut state.0) };
        }
        let n = wtr.finalize()?;
        assert_eq!(n, 0);
        assert_eq!(dst.len(), 0);
